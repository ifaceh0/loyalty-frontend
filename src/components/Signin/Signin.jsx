...
const Signin = () => {
  ...
  const [success, setSuccess] = useState(false);
  const [showPassword, setShowPassword] = useState(false);
  // NEW: Added state to store subscriptionDetails for SHOPKEEPER
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // if (!res.ok) throw new Error(data.message || "Login failed");
      if (!res.ok) {
        const errorMsg = data.message?.toLowerCase() || "login failed";

        // ✅ Specific error messages
        if (errorMsg.includes("No subscription found for the provided email")) {
      ...
      // NEW: Store subscriptionDetails for SHOPKEEPER
      if (role === "SHOPKEEPER" && data.subscriptionDetails) {
        localStorage.setItem("subscriptionDetails", JSON.stringify(data.subscriptionDetails));
      ...
      // setError(err.message);
      // CHANGED: Display specific error messages
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-100 to-purple-200 flex items-center justify-center px-4">
      ...
      <div className="w-full max-w-md backdrop-blur-xl bg-white/60 border border-purple-200 shadow-2xl p-10 rounded-3xl animate-fade-in">
        ...
        {/* NEW: Display subscription details for SHOPKEEPER */}
        {/* {subscriptionDetails && (
          <div className="text-sm text-gray-700 mb-4 text-center">
            Subscription: {subscriptionDetails.planName} ({subscriptionDetails.interval})
          </div>
        )} */}
