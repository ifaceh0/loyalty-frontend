...
export default function InviteEmployeePage() {
  const shopId = 1; // TODO: from JWT / context
        ...
        <div className="max-w-7xl mx-auto p-6 space-y-8">
          ...
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            ...
            <button
              ...
              className="flex items-center gap-2 bg-indigo-600 hover:bg-indigo-700 text-white font-medium px-5 py-3 rounded-xl shadow-lg hover:shadow-xl transition-all"
            >
                ...
                <div className="p-6">
                  {invitations.length === 0 ? (
                    ...
                    <div className="space-y-3">
                      {invitations.map(inv => (
                        <div key={inv.id} className="flex items-center justify-between p-4 bg-slate-50 rounded-xl hover:bg-slate-100 transition">
                          <div className="flex items-center gap-4">
                            <div className="bg-indigo-100 p-2 rounded-lg">
                              {inv.email ? <Mail className="w-5 h-5 text-indigo-600" /> : <Phone className="w-5 h-5 text-indigo-600" />}
                            </div>
                            <div>
                              <p className="font-medium text-slate-900">{inv.email || inv.phone}</p>
                              <p className="text-sm text-slate-500">
                                Expires: {new Date(inv.expiryDate).toLocaleDateString()}
                              </p>
                            </div>
                          </div>
              ...
              <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
                ...
                <div className="p-6">
                  {employees.length === 0 ? (
                    ...
                    <div className="space-y-3">
                      {employees.map(emp => (
                        <div key={emp.userId} className="flex items-center justify-between p-4 bg-slate-50 rounded-xl hover:bg-slate-100 transition">
      ...
      <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} title="Invite New Employee">
        <form onSubmit={handleInvite} className="space-y-5">
          ...
          <button
            ...
            className="w-full bg-indigo-600 hover:bg-indigo-700 disabled:bg-slate-400 text-white font-medium py-3 rounded-xl transition flex items-center justify-center gap-2"
          >
            {loading ? (
              <>
