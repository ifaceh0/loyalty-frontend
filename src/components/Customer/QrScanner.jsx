...
const QrScanner = ({ onClose }) => {
  ...
  return (
    <div className="fixed inset-0 flex items-center justify-center bg-gray-900/90 z-50">
      <div className={`relative w-full px-6 ${scannedData ? 'max-w-2xl' : 'max-w-xl'}`}>
        ...
        {!isPopupOpen && (
          <div className="bg-white shadow-[0_20px_60px_-15px_rgba(0,0,0,0.1)] w-full rounded-[0.5rem] max-h-[90vh] flex flex-col z-10 overflow-hidden transition-all duration-500 border border-slate-100">
            ...
            {/* Clean Header */}
            <nav className="bg-white/80 backdrop-blur-md px-6 py-2 flex justify-between items-center sticky top-0 z-20 border-b border-slate-100 shadow-md">
              ...
              <button
                onClick={handleClose}
                className="group bg-slate-50 text-slate-400 p-3 rounded-full hover:bg-rose-50 hover:text-rose-500 transition-all duration-300"
                aria-label={t("qrScanner.buttons.close")}
              >
            ...
            <div className="p-8 overflow-y-auto flex-1 flex flex-col items-center">
              {!scannedData ? (
                <div className="w-full max-w-sm flex flex-col items-center">
                  ...
                  <div className="relative w-full aspect-square rounded-[2rem] overflow-hidden mb-8 shadow-2xl ring-8 ring-slate-100">
                    <video
                      ref={videoRef}
