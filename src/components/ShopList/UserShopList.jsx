...
//         //   ))}
//         // </div>
//         <div className="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5 gap-2 xl:gap-6 w-full max-w-7xl mx-auto">
//           {currentShops.map((shop) => (
//             <motion.div
//               key={shop.shopId}
//               whileHover={{ y: -8 }}
...
import QRModal from './QRModal';
import { 
    Loader2, Search, X, Store, MapPin, Phone, 
    QrCode, Info, ArrowLeft, ArrowRight, Globe
} from 'lucide-react';
import { FontAwesomeIcon } from '@fortawesome/react-fontawesome';
...
/**
 * PRODUCTION-LEVEL SKELETON LOADER
 * Matches ExploreShops exactly
 */
const ShopSkeleton = () => (
    <div className="bg-white rounded-2xl border border-slate-200 p-4 h-[380px] animate-pulse">
...
export default function UserShopList() {
    ...
    // const currentShops = shops;
    const currentShops = searchTerm ? filteredShops : shops;

    return (
        <div className="min-h-screen bg-[#F9FAFB] text-slate-900 font-sans antialiased">
            
            {/* SaaS Layout Header - Matching ExploreShops */}
                        ...
                        <div>
                            ...
                            <div className="flex items-center gap-2 mt-1">
                                <p className="text-sm text-slate-500">
                                    {t('userShopList.subtitle')}
                                </p>
                                <span className="hidden md:inline w-1 h-1 bg-slate-300 rounded-full mx-1" />
            ...
            <main className="max-w-[1400px] mx-auto px-6 py-8">
                {error && (
                    <div className="bg-red-50 border border-red-100 text-red-600 px-4 py-3 rounded-lg text-sm mb-8 flex items-center gap-2 font-medium">
                        {error}
                    </div>
                )}
                ...
                {isLoading ? (
                    ...
                    /* Empty State - Matching ExploreShops */
                    <div className="flex flex-col items-center justify-center py-24 bg-white rounded-2xl border border-slate-200">
                        <Store className="w-12 h-12 text-slate-200 mb-4" />
                        <h3 className="text-lg font-semibold text-slate-900">{t('userShopList.noResults.oops')}</h3>
                        <p className="text-slate-500 text-sm mt-1">{t('userShopList.noResults.message')}</p>
                    </div>
                ) : (
                    /* Shop Grid - High Quality SaaS Cards */
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5 gap-6">
                        <AnimatePresence mode="popLayout">
                            {currentShops.map((shop) => (
                                ...
                                >
                                    {/* Card Image Section with Protection Overlay */}
                                    <div className="relative h-44 w-full bg-slate-50 rounded-t-xl overflow-hidden border-b border-slate-100">
                                        ...
                                        {/* Status Badge */}
                                        <div className="absolute top-3 left-3 flex items-center gap-1.5 px-2 py-0.5 bg-white/90 backdrop-blur border border-slate-200 rounded-md">
                                            <FontAwesomeIcon icon={faCheckCircle} className="text-emerald-500 text-[10px]" />
                                            <span className="text-[10px] font-bold text-slate-600 uppercase tracking-wider">{t('userShopList.visitedBadge')}</span>
                                        </div>
                                    ...
                                    <div className="p-5 flex flex-col flex-1">
                                        <div className="space-y-3 mb-6">
                                            ...
                                            <div className="flex items-center gap-2 text-slate-500 border-t border-slate-50 pt-3">
                                                ...
                                                <span className="text-xs font-medium">{shop.shopPhone || 'No contact'}</span>
                                            </div>
                                        </div>

                                        {/* Action Footer - Split Actions */}
                                        <div className="mt-auto pt-4 border-t border-slate-50 flex items-center gap-2">
                                            ...
                                            >
                                                {loadingShopId === shop.shopId ? (
                                                    ...
                                                    <><QrCode className="w-3.5 h-3.5" /> {t('userShopList.qr.button')}</>
                                                )}
                                            </motion.button>
                ...
                {/* SaaS Minimalist Pagination - Matching ExploreShops */}
                {totalPages > 1 && (
