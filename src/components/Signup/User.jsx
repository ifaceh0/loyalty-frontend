...
//         setTimeout(() => setError(""), 3000); // Error disappears after 3s
//         return;
//       }
//     } else if (step === 2) {
...
//                 <input name="firstName" value={formData.firstName} onChange={handleChange} className="input peer h-10 border border-gray-300" required />
//                 <label className="floating-label">First Name</label>
//               </div>
//               <div className="relative">
//                 <input name="lastName" value={formData.lastName} onChange={handleChange} className="input peer h-10 border border-gray-300" required />
...
//               <div className="relative">
//                 <input type="tel" name="phoneNumber" value={formData.phoneNumber} onChange={handleChange} className="input peer w-full h-10 border border-gray-300" required />
//                 <label className="floating-label">Phone Number</label>
//               </div>
//               <div className="relative">
//                 <input type="email" name="email" value={formData.email} onChange={handleChange} className="input peer w-full h-10 border border-gray-300" required />
...
//             <div className="grid grid-cols-1 gap-4 animate-fade-in">
//               <div className="relative">
//                 <input type="password" name="password" value={formData.password} onChange={handleChange} className="input peer w-full h-10 border border-gray-300" required />
//                 <label className="floating-label">Password</label>
//               </div>
//               <div className="relative">
//                 <input type="password" name="confirmPassword" value={formData.confirmPassword} onChange={handleChange} className="input peer w-full h-10 border border-gray-300" required />
...
function User() {
  ...
  const nextStep = () => {
    let currentFields = [];
    if (step === 1) currentFields = ["firstName", "lastName"];
    if (step === 2) currentFields = ["phoneNumber", "email"];
      ...
      <div className="bg-white p-8 rounded-2xl shadow-xl w-full max-w-md animate-fade-in">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          {step === 1 && (
            <div className="grid grid-cols-2 gap-4 animate-fade-in">
              <div className="relative">
                <input name="firstName" value={formData.firstName} onChange={handleChange} className={`input peer h-10 transition duration-150 ease-in-out ${missingFields.includes("firstName") ? "ring-2 ring-red-500 shadow-md" : "border border-gray-300"}`} required />
                <label className="floating-label">First Name</label>
              </div>
              <div className="relative">
                <input name="lastName" value={formData.lastName} onChange={handleChange} className={`input peer h-10 transition duration-150 ease-in-out ${missingFields.includes("lastName") ? "ring-2 ring-red-500 shadow-md" : "border border-gray-300"}`} required />
                <label className="floating-label">Last Name</label>
              </div>
              <button type="button" onClick={nextStep} className="col-span-2 bg-blue-500 text-white rounded py-2 hover:bg-blue-600">Next</button>
            </div>
          ...
          {step === 2 && (
            <div className="grid grid-cols-1 gap-4 animate-fade-in">
              <div className="relative">
                <input type="tel" name="phoneNumber" value={formData.phoneNumber} onChange={handleChange} className={`input peer w-full h-10 transition duration-150 ease-in-out ${missingFields.includes("phoneNumber") ? "ring-2 ring-red-500 shadow-md" : "border border-gray-300"}`} required />
                <label className="floating-label">Phone Number</label>
              </div>
              <div className="relative">
                <input type="email" name="email" value={formData.email} onChange={handleChange} className={`input peer w-full h-10 transition duration-150 ease-in-out ${missingFields.includes("email") ? "ring-2 ring-red-500 shadow-md" : "border border-gray-300"}`} required />
                <label className="floating-label">Email</label>
              </div>
              <button type="button" onClick={nextStep} className="bg-blue-500 text-white rounded py-2 hover:bg-blue-600">Next</button>
            </div>
          ...
          {step === 3 && (
            <div className="grid grid-cols-1 gap-4 animate-fade-in">
              ...
              <button type="submit" disabled={loading} className={`w-full flex justify-center items-center bg-purple-600 text-white py-2 rounded-lg transition duration-200 ${loading ? "opacity-70 cursor-not-allowed" : "hover:bg-purple-700"}`}>
                {loading ? (
                  <span className="flex items-center gap-2">
