...
export default function Footer() {
  ...
  return (
    // CHANGED: bg-white -> bg-slate-950 | border-slate-200 -> border-slate-900
    <footer className="relative border-t border-slate-900 bg-slate-950 pt-10 pb-10 overflow-hidden">
      {/* Subtle Background Accent */}
      <div className="absolute top-0 left-1/2 -translate-x-1/2 w-full h-px bg-gradient-to-r from-transparent via-indigo-500/20 to-transparent" />
      ...
      <div className="max-w-7xl mx-auto px-6 lg:px-0">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-12 gap-12 lg:gap-8 mb-10">
          ...
          <div className="lg:col-span-5 space-y-8">
            <div className="flex flex-col items-start gap-4">
              <div className="flex items-center gap-2.5 group cursor-pointer">
                {/* Abstract Icon */}
                {/* CHANGED: border-slate-900 -> border-slate-100 | group-hover:bg-slate-900 -> group-hover:bg-slate-100 */}
                <div className="w-9 h-9 rounded-full border-2 border-slate-100 flex items-center justify-center group-hover:bg-slate-100 transition-all duration-300">
                  {/* CHANGED: text-slate-900 -> text-slate-100 | group-hover:text-white -> group-hover:text-slate-950 */}
                  <span className="text-lg font-black text-slate-100 group-hover:text-slate-950 transition-colors duration-300">
                    LP
                  </span>
                </div>
                ...
                {/* Typography */}
                <div className="flex flex-col leading-none">
                  {/* CHANGED: text-slate-900 -> text-slate-100 */}
                  <span className="text-lg font-extrabold text-slate-100 tracking-widest uppercase">
                    Loyalty
                  </span>
                  {/* CHANGED: text-blue-600 -> text-indigo-400 */}
              ...
              <p className="text-[15px] leading-relaxed text-slate-400 w-full max-auto sm:max-w-sm">
                {t('footer.brandDesc')}
              </p>
            </div>

            {/* Newsletter Input - Restyled for Dark Mode */}
            {/* <div className="w-full max-auto sm:max-w-sm">
              <form onSubmit={handleSubscribe} className="relative group">
                <div className={`relative flex items-center p-1.5 rounded-lg border transition-all duration-300 ${subscribed ? 'border-emerald-500 bg-emerald-950/20' : 'border-slate-800 bg-slate-900/50 group-focus-within:border-indigo-500 group-focus-within:bg-slate-900 group-focus-within:shadow-xl group-focus-within:shadow-indigo-950/30'}`}>
                  ...
                  <input 
                    ...
                    onChange={(e) => setEmail(e.target.value)}
                    placeholder={t('footer.newsletterPlaceholder') || "Enter your email"}
                    className="flex-1 bg-transparent border-none outline-none px-3 text-sm text-slate-100 placeholder:text-slate-500"
                    disabled={subscribed}
                  />
                  <button 
                    type="submit"
                    ...
                    className={`flex items-center justify-center w-9 h-9 rounded-full transition-all duration-300 
                      ${subscribed 
                        ? 'text-emerald-400'
                        : 'text-indigo-400 hover:text-indigo-300 hover:bg-slate-900 active:scale-95'
                      }`}
                  >
                    {subscribed ? <Check size={20} /> : <Send size={20} />}
                  </button>
                ...
                <AnimatePresence>
                  {subscribed && (
                    <motion.p initial={{ opacity: 0, y: 5 }} animate={{ opacity: 1, y: 0 }} className="absolute -bottom-6 left-2 text-[11px] font-bold text-emerald-500 uppercase tracking-wider">
                      Thanks for joining!
                    </motion.p>
                  )}
                </AnimatePresence>
          ...
          {/* Quick Links Column */}
          <div className="lg:col-span-2 md:pl-8">
            {/* CHANGED: text-slate-900 -> text-slate-200 */}
            <h4 className="text-[12px] font-black uppercase tracking-[0.2em] text-slate-200 mb-8">
              {t('footer.quickLinksTitle')}
            </h4>
            <ul className="space-y-4">
              {Array.isArray(quickLinks) && quickLinks.slice(0, 5).map((link) => (
                <li key={link.to}>
                  {/* CHANGED: text-slate-500 -> text-slate-400 | hover:text-indigo-600 -> hover:text-indigo-400 */}
                  <a href={link.to} className="group flex items-center text-[14px] font-medium text-slate-400 hover:text-indigo-400 transition-colors">
                    <ArrowRight size={14} className="opacity-0 -ml-4 group-hover:opacity-100 group-hover:ml-0 transition-all duration-300 mr-2" />
                    {link.label}
                  </a>
                </li>
            ...
            <div className="flex gap-3">
              {socialLinks.map(({ Icon, url, color }, i) => (
                <motion.a 
                  ...
                  key={i} 
                  href={url}
                  // CHANGED: bg-slate-50 -> bg-slate-900 | border-slate-100 -> border-slate-800/50 | text-slate-400 -> text-slate-500
                  className={`w-11 h-11 rounded-2xl bg-slate-900 border border-slate-800/50 flex items-center justify-center text-slate-500 transition-all duration-300 ${color} hover:text-white hover:border-transparent hover:shadow-xl`}
                >
                  <Icon className="w-5 h-5" />
                </motion.a>
              ))}
        ...
        {/* Bottom Bar */}
        {/* CHANGED: border-slate-100 -> border-slate-900 */}
        <div className="border-t border-slate-900 pt-6 flex flex-col md:flex-row justify-between items-center gap-6">
          <div className="flex flex-col md:flex-row items-center gap-2 md:gap-6 text-center md:text-left">
            ...
            {/* CHANGED: bg-slate-200 -> bg-slate-800 */}
            <div className="hidden md:block w-1 h-1 rounded-full bg-slate-800" />
            <div className="flex gap-6 text-[13px] font-bold">
              {/* CHANGED: text-slate-400 -> text-slate-500 | hover:text-indigo-600 -> hover:text-indigo-400 */}
              <a href="/privacy" className="text-slate-500 hover:text-indigo-400 transition-colors">{t('footer.privacy')}</a>
              <a href="/terms" className="text-slate-500 hover:text-indigo-400 transition-colors">{t('footer.terms')}</a>
            </div>
          </div>
          
          {/* CHANGED: bg-slate-50 -> bg-slate-900 | border-slate-100 -> border-slate-800/60 */}
          <div className="flex items-center gap-1 bg-slate-900 px-3 py-1.5 rounded-full border border-slate-800/60">
            <div className="w-1.5 h-1.5 rounded-full bg-emerald-500 animate-pulse" />
            {/* CHANGED: text-slate-500 -> text-slate-400 */}
